% Periodically driven-dissipatie free fermion
% Two 1D chains with pairing in the bulk - equivalent to XY model
...
Hc = zeros(L);%hopping Hamiltonian in canonical fermions
% alpha = 0.85;
for j = 1:L-2
    Hc(j,j+2) = J;
    Hc(j+2,j) = J';
...
for j = 1:L
    H0(2*j-1,2*j) = H0(2*j-1,2*j)-1i*h/2;
    H0(2*j,2*j-1) = H0(2*j,2*j-1)+1i*h/2;
end

% pairing in the bulk pairs (j,up) with (j+1,up), and same for down.
% Upon (j,up)-> 2j-1, (j,down)-> 2j,
% We pair 2j-1 with 2j+1, 2j with 2j+2
% We need to further write everything in Majorana basis
pair = 0.5;
Hp = zeros(2*L);
for j = 1:Lspin-1
    %spin up part
    Hp(4*j-3,4*j+1) = (pair-pair')/8;
    Hp(4*j-3,4*j+2) = 1i*(pair+pair')/8;
    Hp(4*j-2,4*j+1) = 1i*(pair+pair')/8;
    Hp(4*j-2,4*j+2) = -(pair-pair')/8;
    %spin down part
    Hp(4*j-1,4*j+3) = (pair-pair')/8;
    Hp(4*j-1,4*j+4) = 1i*(pair+pair')/8;
    Hp(4*j,4*j+3) = 1i*(pair+pair')/8;
    Hp(4*j,4*j+4) = -(pair-pair')/8;
...
Gamma1 = sqrt(0.1); l(1,1) = Gamma1*1/2; l(1,2) = Gamma1*1i/2;%L1 = c1^\dag
Gamma2 = sqrt(0.1); l(2,1) = Gamma2*1/2; l(2,2) = -Gamma2*1i/2;%L2 = c1
Gamma3 = sqrt(0.1); l(3,3) = Gamma3*1/2; l(3,4) = Gamma3*1i/2;%L3 = c2^\dag
Gamma4 = sqrt(0.1); l(4,3) = Gamma4*1/2; l(4,4) = -Gamma4*1i/2;%L4 = c2
Gamma5 = sqrt(0.1); l(5,2*L-3) = Gamma5*1/2;  l(5,2*L-2) = Gamma5*1i/2;%L5 = c{L-1}^\dag
Gamma6 = sqrt(0.1); l(6,2*L-3) = Gamma6*1/2;  l(6,2*L-2) = -Gamma6*1i/2;%L6 = c{L-1}
Gamma7 = sqrt(0.1); l(7,2*L-1) = Gamma7*1/2;  l(7,2*L) = Gamma7*1i/2;%L7 = cL^\dag
Gamma8 = sqrt(0.1); l(8,2*L-1) = Gamma8*1/2;  l(8,2*L) = -Gamma8*1i/2;%L8 = cL
...
tmax = 3000; % a long enough time for the system to relax
tst = 100; %5*2*pi/omega;
...
occ = zeros(length(t),1);
for j = 1:length(t)
    occ(j) = 1/2 + 1i*C(j,2*site,2*site-1)/2;
end
...
figure
plot(t(3:length(t)),occ(3:length(t)))
title("<n_j>(t)")
xlabel("t")
ylabel("<n_j>")
...
Ccorravg = Ccorravg([1:2:L 2:2:L],[1:2:L 2:2:L]);
xx=kron(0:L-1,[1,0])+kron(1:L,[0,1]);
[X,Y]=meshgrid(xx,xx);
Z=kron(log10(abs(Ccorravg)),[1,1;1,1]);% log scale
...
caxis([-10,-8]);colormap('jet');colorbar;
